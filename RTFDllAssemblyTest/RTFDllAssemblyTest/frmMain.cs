...
namespace RTFDllAssemblyTest
{
    public partial class frmMain : Form
    {
        ...
        private void ConstructNodeStruct(TreeNode tnParent, RTFDomElement element)
        {
            ...
            foreach (RTFDomElement nextLevelElement in element.Elements)
            {
                if (nextLevelElement is RTFDomImage)
                {
                    //MessageBox.Show((nextLevelElement as RTFDomImage).ToString());
                }

                TreeNode tnNextLevel = new TreeNode(nextLevelElement.GetType().ToString());
                tnNextLevel.ToolTipText = nextLevelElement.InnerText;
                tnParent.Nodes.Add(tnNextLevel);
                ConstructNodeStruct(tnNextLevel, nextLevelElement);
            }
        }
        ...
        private void btnCreateRtfContent_Click(object sender, EventArgs e)
        {
            ...
            table.setInnerBorder(Elistia.DotNetRtfWriter.BorderStyle.Dotted, 1f);
            table.setOuterBorder(Elistia.DotNetRtfWriter.BorderStyle.Single, 2f);
            ...
            // RtfDocument objects.
            txtRtfWrtterResult.Rtf = doc.render();
            Clipboard.SetData(DataFormats.Rtf, txtRtfWrtterResult.Rtf);
            //ms.Dispose();
            //b.Dispose();
        }
    }
